...
namespace Gaskellgames.EditorOnly
{
    ...
    public class GgCoreSamplesEditorWindow : GgEditorWindow_MenuTree
    {
        ...
        {
            if (GUILayout.Button("MethodRow1")) { Log(LogType.Log, "Method 1: Button Pressed"); }
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("MethodRow2a")) { Log(LogType.Log, "Method 2a: Button Pressed"); }
            if (GUILayout.Button("MethodRow2b")) { Log(LogType.Log, "Method 2b: Button Pressed"); }
            EditorGUILayout.EndHorizontal();
        }
        
        private void DrawScriptableObjectEditor(Editor scriptableObjectEditor)
        {
            if (scriptableObjectEditor) { scriptableObjectEditor.OnInspectorGUI(true, true); }
