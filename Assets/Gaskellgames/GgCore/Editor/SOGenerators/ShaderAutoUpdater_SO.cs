...
namespace Gaskellgames.EditorOnly
{
    ...
    public class ShaderAutoUpdater_SO : GgScriptableObject
    {
        ...
        private void UpdateMaterials(Pipeline pipeline)
        {
            switch (pipeline)
            {
                case Pipeline.BIRP:
                    if (!BIRP)
                    {
                        Log(LogType.Warning, "Warning: Missing shader reference: ShaderAutoUpdater failed to update shaders for the current pipeline.");
                        break;
                    }
                    for (int i = materials.Count - 1; i >= 0; i--)
                    {
                        ...
                        Log(LogType.Log, "Material [{0}] updated to use shader [{1}].", materials[i].name, BIRP);
                    }
                    break;
                ...
                case Pipeline.URP:
                    ...
                    for (int i = materials.Count - 1; i >= 0; i--)
                    {
                        ...
                        Log(LogType.Log, "Material [{0}] updated to use shader [{1}].", materials[i].name, URP);
                    }
                    break;
                ...
                case Pipeline.HDRP:
                    ...
                    for (int i = materials.Count - 1; i >= 0; i--)
                    {
                        ...
                        Log(LogType.Log, "Material [{0}] updated to use shader [{1}].", materials[i].name, HDRP);
                    }
                    break;
                ...
                default:
                    Log(LogType.Warning, "Warning: Other or Unknown pipeline detected. ShaderAutoUpdater failed to update shaders for the current pipeline.");
                    break;
            }
        }
        ...
        private Pipeline GetTargetRenderPipeline()
        {
            ...
            {
                Log(LogType.Log, "Using override render pipeline: {0}", pipeline);
                return pipeline;
            }
            
            pipeline = GetDefaultRenderPipeline();
            Log(LogType.Log, "Using default render pipeline: {0}", pipeline);
            return pipeline;
        }
