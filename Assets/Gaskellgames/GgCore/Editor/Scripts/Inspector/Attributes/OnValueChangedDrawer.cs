        ...
        protected override void OnGgGUI(Rect position, SerializedProperty property, GUIContent label, GgGUIDefaults defaultCache)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                foreach (Object target in targets)
                {
                    ...
                    if (method == null)
                    {
                        GgLogs.Log(null, LogType.Error, "Unable to find method '{0}' on type '{1}'.", attributeAsType.methodName, type.Name);
                    }
                    else if (0 < method.GetParameters().Length)
                    {
                        GgLogs.Log(null, LogType.Error, "Method '{0}' on type '{1}' cannot be called as it requires at least one parameter.", attributeAsType.methodName, type.Name);
                    }
                    else
                    {
