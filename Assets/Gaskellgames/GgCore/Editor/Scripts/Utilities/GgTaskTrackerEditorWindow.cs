...
namespace Gaskellgames
{
    ...
    public class GgTaskTrackerEditorWindow : GgEditorWindow
    {
        ...
        protected override void OnPageGUI()
        {
            ...
            // draw table
            DrawInfo();
            EditorExtensions.DrawInspectorLineFull(InspectorExtensions.backgroundSeperatorColor, 2, 6);
            DrawTable();
        }
        ...
        {
            GgTask.SortBy = (GgTask.SortingType)columnHeader.sortedColumnIndex;
            GgTask.IsSortedAscending = columnHeader.IsSortedAscending(columnHeader.sortedColumnIndex);
        }
        ...
        private void DrawInfo()
        {
            ...
            if (GUILayout.Button(new GUIContent("Resize Table", "Resizes the table width to fit the window width."), EditorStyles.toolbarButton))
            {
                InitialiseTable();
            }
            if (GUILayout.Button(new GUIContent("Cancel All", "Cancel all 'InProgress' GgTasks."), EditorStyles.toolbarButton))
            {
            ...
            if (GUILayout.Toggle(GgTask.DebugMode, new GUIContent(" Debug", iconTexture, "Keep a record of all tasks for debugging."), EditorStyles.toolbarButton))
            {
                if (!changeCheck)
                {
        ...
        private void DrawTable()
        {
            ...
            Rect headerRect = new Rect(standardSpacing, yPosition, pageWidth - (standardSpacing * 2) - scrollBarOffset, columnHeader.height);
            
            // draw the column headers
            Color32 defaultBackground = GUI.backgroundColor;
            GUI.backgroundColor = InspectorExtensions.backgroundHoverColor;
